  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 sm:px-6 space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-6 sm:p-8 shadow-xs">
        ...
        {/* Contributor External Links (From Firestore) */}
        {user.links && (user.links.website || user.links.docs || user.links.twitter || user.links.github) && (
          <div className="mt-4 pt-4 border-t border-stone-100 flex flex-wrap items-center gap-3 text-xs">
            <span className="text-[11px] font-bold text-stone-500 uppercase tracking-wider">Links:</span>
            {user.links.website && (
              <a
                href={user.links.website}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1 text-stone-700 hover:text-stone-950 font-medium underline underline-offset-2"
              >
        ...
        {/* Stats & Firestore Status Row */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-stone-100 text-xs">
          <div>
          ...
          <div>
            <span className="text-stone-500 block">Firestore Account</span>
            <span className="text-xs font-semibold text-emerald-800 font-mono mt-0.5 inline-flex items-center gap-1 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
              <Database className="w-3 h-3 text-emerald-600" />
              <span>users/{user.id.slice(0, 10)}</span>
            </span>
          </div>
        </div>
      </div>
